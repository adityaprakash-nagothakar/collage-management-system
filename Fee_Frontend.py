    ...
    def __init__(self, master):
        ...
        def Insert():
            if (len(self.admsn.get()) != 0):
                Fee_Frontend.insert(self.recpt.get(), self.name.get(), self.admsn.get(), self.date.get(),
        ...
        def Search():
            self.list.delete(0, END)
            for row in Fee_Frontend.search(self.recpt.get(), self.name.get(), self.admsn.get(), self.date.get(),
                                          self.branch.get(), self.sem.get(), self.total.get(), self.paid.get(),
                                          self.due.get()):
                self.list.insert(END, row, str(' '))
